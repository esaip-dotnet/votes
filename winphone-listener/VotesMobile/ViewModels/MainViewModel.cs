...
using VotesMobile.Models;
namespace VotesMobile.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private void webClient_DownloadCatalogCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            try
            {
                ...
                if (e.Result != null)
                {
                    //Elections B1 = new Elections();
                    //votes v1 = new votes();

                    var Elections = JsonConvert.DeserializeObject<Elections[]>(e.Result);
                    int id = 0;
                    foreach (Elections book in Elections)
                    {
